...
public class CashActivity2 extends AbsActivity implements View.OnClickListener {
    ...
    private void setCash() {
        ...
        if (TextUtils.isEmpty(money)) {
            ...
            Toast.makeText(mContext, getResources().getString(R.string.请输入提现金额), Toast.LENGTH_LONG).show();
            return;
        }
        double moneyVal = 0;
        try {
            ...
            if (moneyVal > mRmbVal) {
                ...
                Toast.makeText(mContext, getResources().getString(R.string.余额不足), Toast.LENGTH_LONG).show();
                return;
            }
            if (moneyVal < mMinCashCoinVal) {
                ...
                Toast.makeText(mContext, getResources().getString(R.string.提现金额不能小于) + mMinCashCoinVal + getResources().getString(R.string.元), Toast.LENGTH_LONG).show();
                return;
            }
        } catch (Exception e) {
            //ToastUtil.show(getResources().getString(R.string.请输入正确的金额));
            Toast.makeText(mContext, getResources().getString(R.string.请输入正确的金额), Toast.LENGTH_LONG).show();
            return;
        }
        String account = mAccountInput.getText().toString().trim();
        if (TextUtils.isEmpty(account)) {
            //ToastUtil.show(getResources().getString(R.string.请输入提现账号));
            Toast.makeText(mContext, getResources().getString(R.string.请输入提现账号), Toast.LENGTH_LONG).show();
            return;
        }
