...
public class VideoRecordActivity extends AbsActivity {
    ...
    private void initCameraRecord() {
        //TXUGCBase.getInstance().setLicence(getApplicationContext(), "http://license.vod2.myqcloud.com/license/v1/3b39efa2d155407693cb7ce699b61f29/TXUgcSDK.licence", "89d80be400006120e36b5c2afae58497");
        TXUGCBase.getInstance().setLicence(getApplicationContext(), "https://license.vod2.myqcloud.com/license/v2/1300345353_1/v_cube.license", "5776d2ddaef36877372fc4f23e5ae5dd");
    ...
    private TXRecordCommon.ITXVideoRecordListener mITXVideoRecordListener = new TXRecordCommon.ITXVideoRecordListener() {
        ...
        public void onRecordEvent(int event, Bundle bundle) {
            Log.e("#onRecordComplete--   ", Integer.toString(event));
        ...
        public void onRecordComplete(TXRecordCommon.TXRecordResult result) {
            ...
            Log.e("#onRecordComplete", "#时间戳------->1   " + time);
            Log.e("#onRecordComplete", "retCode------->     " + mTXRecordResult.retCode);
            if (mTXRecordResult.retCode < 0) {
                Log.e("#onRecordComplete", "#时间戳------->2   " + time);
                ToastUtil.show(WordUtil.getString(R.string.record_failed) + " == " + 1);
                return;//不写这个的话，有时候会执行下面的else
            } else {
                Log.e("#onRecordComplete", "#时间戳------->3   " + time);
                long duration = 0;
                if (mTXCameraRecord != null) {
