...
public class VideoEditActivity extends AbsActivity implements View.OnClickListener {
    ...
    protected void main() {
        ...
        if (editer == null){// || editer.getTXVideoInfo() == null) {
            ...
            Toast.makeText(mContext, WordUtil.getString(R.string.video_edit_status_error), Toast.LENGTH_LONG).show();
            finish();
            return;
        }
...
*/
    ...
    public void editClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.btn_cover://封面
                ...
                Toast.makeText(mContext, getResources().getString(R.string.敬请期待), Toast.LENGTH_LONG).show();
                break;
    ...
    private void generateVideo() {
        Log.e(TAG, "#toNext------->生成视频");
        if (mTXVideoEditer == null) {
            return;
        }
        if (mCutEndTime - mCutStartTime < 1000) {
            //ToastUtil.show(WordUtil.getString(R.string.video_duration_too_short));
            Toast.makeText(mContext, WordUtil.getString(R.string.video_duration_too_short), Toast.LENGTH_LONG).show();
        }
    ...
    private TXVideoEditer.TXVideoGenerateListener mTXVideoGenerateListener = new TXVideoEditer.TXVideoGenerateListener() {
        ...
        public void onGenerateComplete(TXVideoEditConstants.TXGenerateResult result) {
            Log.e(TAG, "#onGenerateComplete------->");
            if (result.retCode == TXVideoEditConstants.GENERATE_RESULT_OK) {
                Log.e(TAG, "#onGenerateComplete------->生成视频成功");
                saveGenerateVideoInfo();
                //ToastUtil.show(WordUtil.getString(R.string.generate_video_success));

                Toast.makeText(mContext, WordUtil.getString(R.string.generate_video_success), Toast.LENGTH_LONG).show();
    ...
    private void onGenerateFailure() {
        ...
        Toast.makeText(mContext, WordUtil.getString(R.string.generate_video_failed), Toast.LENGTH_LONG).show();
    ...
    private void generateCoverFile() {
        Log.e(TAG, "#generateCoverFile------->生成视频的封面图");
        new AsyncTask<Void, String, String>() {
            ...
            protected void onPostExecute(String coverFilePath) {
                Log.e(TAG, "#generateCoverFile------->coverFilePath  " + coverFilePath);
    ...
    private void cancelGenerateVideo() {
        ...
        Toast.makeText(mContext, WordUtil.getString(R.string.cancel_generate_video), Toast.LENGTH_LONG).show();
    }
